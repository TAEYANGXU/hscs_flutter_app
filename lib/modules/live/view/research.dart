...
class _ResearchPageState extends State<ResearchPage> {
  ...
  Widget _cellForRow(BuildContext context,int index){
    ...
    return GestureDetector(
      onTap: (){
        Routers.push(context, WebViewRouter.webView,params: {"url":model.detailLink});
      },
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: CacheImage(imageUrl: model.coverImage,width: double.infinity,height: Adapt.px(140),),
            ),
            Container(
              margin: EdgeInsets.all(Adapt.px(15)),
              child: Text(model.title!,style: const TextStyle(fontSize: TextSize.big,color: AppColors.text,fontWeight: FontWeight.bold),),
            ),
            Container(
              width: double.infinity,
              child: Row(
                children: [
                  Expanded(
                    ...
                    child: Container(
                      ...
                      child: Text(formatDate(DateTime.fromMillisecondsSinceEpoch(model.createTime! * 1000), [yyyy, '-',mm, '-',dd,' ',HH,':',nn]) ?? ""),
                    ),
                  ),
                  Expanded(flex: 1,child: Row(
                    ...
                    children: [
                      Container(
                        child: const Text("查看详情",style: TextStyle(color: AppColors.theme),),
                      ),
