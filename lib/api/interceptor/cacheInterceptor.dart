...
class NetCacheInterceptor extends Interceptor {
  ...
  void onRequest(RequestOptions options,
      RequestInterceptorHandler requestCb,) async {
    if (!CACHE_ENABLE) {
    ...
    if (refresh) {
      ...
      // 删除磁盘缓存
      if (cacheDisk) {
        // await CacheData().remove(options.uri.toString());
      }
    ...
    if (options.extra["noCache"] != true &&
      ...
      // 2 磁盘缓存
      if (cacheDisk) {
        var cacheData = CacheData().getJSON(key);
        if (cacheData != null) {
          return;
        }
      }
  ...
  void onResponse(Response response,
      ResponseInterceptorHandler responseCb) async {
    // 如果启用缓存，将返回结果保存到缓存
    if (CACHE_ENABLE) {
  ...
  Future<void> _saveCache(Response object) async {
    ...
    if (options.extra["noCache"] != true &&
      ...
      if (options.extra["cacheDisk"] == true) {
        // await CacheData().setJSON(key, object.data);
      }
